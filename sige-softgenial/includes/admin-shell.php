        ...
        <aside id="sige-sidebar" class="sg-app-sidebar" style="--sg-school-primary: <?php echo esc_attr($escola->cor_primaria ?: 'var(--sg-theme-primary-800,#3b2f8d)'); ?>;">
            ...
            <nav class="sg-app-nav">
                ...
                <?php if (empty($sige_scoped_professor) && $ff_academico && $sige_menu_can_any(['academico.disciplinas_ver','academico.matriz_ver','academico.turmas_ver','academico.boletins_ver','academico.fechar_ano','academico.reabrir_ano','alunos.contas_ver','configuracoes.ver'])): ?>
                    ...
                    <?php if ($sige_menu_can_any(['alunos.contas_ver'])): ?><a href="?page=sige-app&view=aluno_contas" class="sige-menu-item <?php echo $view == 'aluno_contas' ? 'active' : ''; ?>"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="sige-u-shrink-0"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg> Contas de Alunos</a><?php endif; ?>
                <?php endif; ?>
                <?php // M6 (v12.19.9): grupo PORTARIA fundido em "OPERAÇÃO ESCOLAR" (mais abaixo) para evitar grupos de um so item. ?>
                ...
                <?php if ($ff_financeiro && $mod_financeiro_ativo): ?>
                    <?php
                    // v12.19.11 - Tesouraria reagrupada por dominio (4 grupos): TESOURARIA
                    // (caixa: entradas/saidas), FATURACAO (cobrancas a alunos), RELATORIOS &
                    // CONTROLO, e CONFIGURACAO. Cada item mantem a sua propria guarda de
                    // permissao (igual ao $sige_view_permission_map). So apresentacao/IA.
                    $sg_fin_mpesa = function_exists('sige_mpesa_pode_gerir') && sige_mpesa_pode_gerir();
                    $sg_fin_aprov = function_exists('sige_fin_aprovacao_pode_aceder') && sige_fin_aprovacao_pode_aceder();
                    $sg_grp_tesouraria = $sige_menu_can_any(['financeiro.pagar','financeiro.cobrancas_ver','financeiro.extractos_ver','financeiro.despesas_ver']) || $sg_fin_mpesa;
                    $sg_grp_faturacao = $sige_menu_can_any(['financeiro.lancar_mensalidades','financeiro.lancamentos_ver','financeiro.lancamentos_gerir','financeiro.planos_ver','financeiro.planos_gerir']);
                    $sg_grp_relatorios = $sige_menu_can_any(['financeiro.dashboard_ver','financeiro.ver','financeiro.relatorio_mensal_ver','financeiro.pagamentos_turma_ver','financeiro.auditoria_ver']) || $sg_fin_mpesa || $sg_fin_aprov;
                    $sg_grp_fin_config = $sige_menu_can_any(['financeiro.servicos_ver','financeiro.configurar_precos','financeiro.centros_custo_ver']);
                    ?>
                    <?php if ($sg_grp_tesouraria): ?>
                    ...
                    <?php endif; ?>
                    <?php if ($sg_grp_relatorios): ?>
                    <div class="menu-label">RELATÓRIOS &amp; CONTROLO</div>
                ...
                <?php endif; ?>
                <!-- OPERAÇÃO ESCOLAR - Portaria + Transporte (M6: grupos de 1 item fundidos) -->
                <?php
