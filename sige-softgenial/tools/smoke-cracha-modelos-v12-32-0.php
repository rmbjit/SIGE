...
_p($fails, strpos($view, 'assets/cracha/sige-cracha-templates.js') !== false, 'Registo inline lê de assets/cracha/ (localização validada)');
_p($fails, is_file($root . '/assets/cracha/sige-cracha-templates.js'), 'Ficheiro do registo está em assets/cracha/');
// Aurora: a foto não pode tapar o título -> padding inferior do cabeçalho > sobreposição da foto.
_p($fails, strpos($js, 'padding:12px 8px 46px') !== false && strpos($js, 'margin-top:-40px') !== false, 'Aurora: padding inferior (46) > sobreposição da foto (40) -> sem overlap do título');
