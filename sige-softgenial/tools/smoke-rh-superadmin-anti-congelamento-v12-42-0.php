<?php
/**
 * Smoke v12.42.0 - (1) super admin fora de Ausencias/Assiduidade/Salarios;
 * (2) correccao rigorosa do congelamento (fonte de verdade unica do shell +
 * rede em pointerdown).
 *
 * (1) DADOS: com stubs de get_user_by/sige_is_real_wp_admin_user (a MESMA
 *     ligacao por email que a aba Equipa usa), sige_rh_professor_e_admin_sistema
 *     reconhece o admin WP real e sige_rh_salario_mapa_mes() SALTA a linha cujo
 *     email pertence ao admin.
 * (2) LIGACAO: dropdown de Ausencias, grelha de Assiduidade e preview/mapa de
 *     Salarios seleccionam email e aplicam a exclusao; o reconciliador unico do
 *     shell existe e esta ligado (open/close, cracha, pointerdown de captura);
...
// Resolucao do admin WP real por EMAIL (mesmo criterio da aba Equipa).
if (!function_exists('get_user_by')) {
    function get_user_by($field, $value) {
        if ($field === 'email' && strtolower((string) $value) === 'admin@sistema.mz') {
            $u = new stdClass(); $u->ID = 1; $u->user_email = 'admin@sistema.mz'; return $u;
        }
        return false;
    }
}
...
_p($fails, function_exists('sige_rh_professor_e_admin_sistema') && sige_rh_professor_e_admin_sistema('ADMIN@sistema.MZ'), 'Reconhece o admin por email (case-insensitive)');
...
_p($fails, strpos($assi, 'function sige_rh_professor_e_admin_sistema') !== false, 'Helper unico definido em rh-assiduidade.php');
_p($fails, strpos($assi, "get_user_by('email'") !== false, 'Helper resolve o utilizador por email (mesmo criterio da Equipa)');
...
_p($fails, strpos($equipe, 'sige_rh_professor_e_admin_sistema($__p->email') !== false, 'Dropdown de Ausencias aplica a exclusao');
...
_p($fails, !empty($vh[1]) && ($vh[1] === ($vc[1] ?? '')) && (($vc[1] ?? '') === $vb) && $vb === '12.42.1', 'Versoes sincronizadas em 12.42.1 -> ' . ($vh[1] ?? '?'));
