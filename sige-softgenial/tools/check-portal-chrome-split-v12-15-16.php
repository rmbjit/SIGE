...
/**
 * SIGE SoftGenial v12.15.16 - Gate do split chrome (Fase 2 fechada em producao).
 *
 * Sucede ao gate v12.15.15. Mantem a proteccao de drift (folha comprometida ==
 * saida do gerador) e passa a exigir a flag de chrome LIGADA por defeito, com
 * reversibilidade preservada (definir a option a '0' repoe o style.css completo).
 *
 * Regenerar a folha: php tools/gen-portal-chrome.php
 */
...
foreach ([$lib => 'tools/lib-portal-chrome.php', $hdr => 'tools/portal-chrome-header.txt', $chromePath => 'assets/style-portal-chrome.css'] as $path => $label) {
    if (!is_file($path)) {
        fwrite(STDERR, "check-portal-chrome-split-v12-15-16: FALHOU\n- Ficheiro em falta: {$label}\n");
        exit(1);
    }
}
require_once $lib;
...
// 1. DRIFT: a folha comprometida tem de ser, byte-a-byte, a saida do gerador.
$expected = null;
try {
...
// 2. Dimensao sensata e menor que o completo.
...
// 4. Ligacao na shell: flag LIGADA por defeito (Fase 2 fechada), mas reversivel.
if (strpos($shell, "get_option('sige_portal_chrome_css_v121514_enabled', '1') === '1'") === false) {
    $errors[] = 'A flag de chrome nao esta ligada por defeito na shell (esperado default \'1\').';
}
...
if (strpos($shell, "SIGE_URL . 'assets/style.css'") === false) {
    $errors[] = 'A shell deixou de enfileirar o style.css completo (ramo de fallback e default para staff/outras views).';
}
...
printf("check-portal-chrome-split-v12-15-16: OK - folha gerada sincronizada (%d KB de %d KB), sem views financeiras, ligada por defeito e reversivel.\n", (int) round($chrome_bytes / 1024), (int) round($full_bytes / 1024));
