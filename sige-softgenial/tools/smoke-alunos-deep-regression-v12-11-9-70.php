...
/**
 * Smoke test v12.11.9.70 - Alunos Deep Regression + Bottom Nav Hotfix
...
$checks = [
    ...
    'bottom_nav_mais_button_v70' => strpos($alunos, 'class="sige-mobile-more-trigger"') !== false && strpos($alunos, 'data-sige-mobile-more="1"') !== false,
    'bottom_nav_more_scope_guarded_v70' => strpos($alunos, '$sige_alunos_can_more_nav = !$sige_alunos_is_guarda') !== false && strpos($alunos, 'if ($sige_alunos_can_more_nav)') !== false,
    'bottom_nav_more_inline_fallback_v70' => strpos($alunos, 'sige-alunos-bottom-more-menu-v1211970') !== false && strpos($alunos, 'window.sigeAlunosToggleMoreMenu') !== false,
    'bottom_nav_mais_not_config_link_v70' => strpos($alunos, "<span>Mais</span>
    </a>
    <?php endif; ?>") === false,
    'asset_local_more_selector_v70' => strpos($asset, '.sige-mobile-bottom-nav button[aria-controls="sige-sidebar"]') !== false,
    'asset_local_more_toggles_sidebar_v70' => strpos($asset, "sidebarState(!(side && side.classList.contains('open')))") !== false,
...
if ($failed) {
    fwrite(STDERR, 'Smoke Alunos Deep Regression v12.11.9.70 falhou: ' . implode(', ', $failed) . PHP_EOL);
    exit(1);
}
echo 'Smoke Alunos Deep Regression v12.11.9.70 OK: ' . count($checks) . '/' . count($checks) . ' checks.' . PHP_EOL;
