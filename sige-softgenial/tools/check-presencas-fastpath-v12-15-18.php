 ...
 * atras da flag sige_presencas_fast_path_v121517_enabled, agora LIGADA por defeito (
 ...
 *  (d) view: flag LIGADA por defeito e reversivel (a '0'), com o markup do
...
foreach ([
    ...
    if (!is_file($path)) {
        fwrite(STDERR, "check-presencas-fastpath-v12-15-18: FALHOU\n- Ficheiro em falta: {$label}\n");
        exit(1);
    }
}
...
// (d) View: flag LIGADA por defeito, reversivel, com markup do caminho rapido.
if (strpos($view, "get_option('sige_presencas_fast_path_v121517_enabled', '1') === '1'") === false) {
    $errors[] = 'A flag do caminho rapido nao esta ligada por defeito na view (esperado default \'1\', reversivel a \'0\').';
}
foreach ([
...
printf("check-presencas-fastpath-v12-15-18: OK - caminho rapido LIGADO por defeito e reversivel, handler de lote com guarda completa, sem dialogos nativos nem onclick, manifesto e CSS alinhados.\n");
