# Cenarios de teste live - SIGE SoftGenial v12.12.63
...
Como o shell admin e toda a interface e nao e testavel em browser daqui, seguiu-se o caminho profissional para uma superficie grande: Report-Only primeiro. Acrescentou-se, via admin_init e so nas paginas page=sige-app (antes do output, protegido por headers_sent e nao-ajax), um cabecalho Content-Security-Policy-Report-Only com apenas script-src self mais nonce por pedido. Report-Only nao bloqueia nada: a consola do browser passa a reportar as violacoes reais (handlers inline e scripts sem nonce) que faltam tratar antes de passar a enforce. So o script-src e restringido; os restantes tipos de recurso ficam sem restricao, para a consola so reportar o que interessa.

Confirmou-se que os 2 onclick do ficheiro canonico finance-core estao em sige_desp_print_page, uma pagina standalone de impressao (DOCTYPE proprio), fora do shell admin, pelo que nao caem sob este CSP nem precisam de ser tocados.

Em paralelo, converteram-se 21 onclick das vistas admin para data-sige-act:
- 18 com contratos ou wrappers ja existentes: window.print via sigeImprimirPagina; activarTab por data-sige-args; filtrarTabela com data-sige-self; resetSenha e gerirHorario por inteiro; anularLote sem argumentos; carregarHistorico com booleano; o fecho de popup backdrop via sigeFecharPopupBackdrop.
- 3 com dois wrappers novos: sigeAlternarDisplay (alterna a visibilidade por id, data-target, entre o valor de exibicao data-display e none; usado em dois toggles) e sigeMarcarDownloadSemTransicao.

Comportamento identico. A catraca check-inline-frontend desce o maximo de onclick de 66 para 45 e passa a exigir o CSP Report-Only com script-src nonce no shell admin.

Ficam para os proximos passos, e serao reportados pela propria consola via Report-Only, os onclick restantes das vistas admin: os window.open(this.href) e this.form, e os gerados dentro de blocos script. style mantem-se em 2051 e blocos script em 7. Sem nova superficie de accao (199, enforce 33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo financeiro byte-identico, dependencias externas em 9.

Testes especificos desta versao: o shell admin (page=sige-app) passa a enviar um cabecalho Content-Security-Policy-Report-Only com script-src self mais nonce, antes do output, so nessas paginas e nao em ajax; como e Report-Only, nada e bloqueado e o admin funciona na integra, mas a consola do browser reporta os handlers inline e scripts sem nonce que faltam tratar. Os 21 botoes convertidos chamam as mesmas funcoes pelos contratos do despachante (sigeImprimirPagina, activarTab, filtrarTabela com o elemento, resetSenha, gerirHorario, anularLote, carregarHistorico, sigeFecharPopupBackdrop) e pelos novos wrappers (sigeAlternarDisplay com data-target e data-display, sigeMarcarDownloadSemTransicao). O sige-ui.js passa o node -c. Os verificadores de JavaScript embebido validam as vistas admin sem erro de sintaxe. A catraca trava onclick em 45 (sem folga) e exige o Report-Only no shell. php -l limpo nos ficheiros tocados. Corredor 99, regression-pack verde.
