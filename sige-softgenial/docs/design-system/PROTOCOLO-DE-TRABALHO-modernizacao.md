...
| Criei a funcionalidade de modelos de crachá num **asset numa pasta NOVA** (`assets/cracha/`). No servidor do utilizador (CloudPanel) a pasta nova **nunca foi criada** ao actualizar → o registo JS nunca carregava (nem por HTTP, nem o fallback `readfile`, pois o ficheiro-fonte também não existia lá). Pré-visualização "indisponível" e impressão em fallback, apesar de o código estar correcto (provado em laboratório) | **Não introduzir pastas novas** para entregar código: muitos pipelines de update (CloudPanel, rsync sem `--delete`/sem novos caminhos, extracção parcial de ZIP) **só substituem ficheiros em pastas existentes** e ignoram directórios novos. Colocar assets novos em **pastas que já existem e já deployam** (ex.: `assets/views/`) ou **embutir** o conteúdo num ficheiro PHP existente. Quando uma feature depende de um ficheiro novo, **confirmar a entrega real** (ver-fonte/Console: o recurso existe? a global está definida?), não assumir que o ZIP basta. Diagnóstico rápido: `typeof window.X` na consola distingue "não-deployado" de "deployado-mas-com-bug" |
