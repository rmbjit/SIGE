...
| ESC fechava sempre um modal fixo (`fecharModal`), deixando os outros (importação/360) visíveis mas a cair atrás da sidebar; e um popup de confirmação aparecia **atrás** do modal que o abriu (z-index menor) | Handlers de fecho (ESC, clique-fora) têm de visar **o modal que está aberto**, não um fixo. Quando há um ecrã com vários modais/overlays, verificar cada um (abrir, ESC, clique-fora, fechar) e garantir **ordem de z-index coerente** entre overlays sobrepostos (um popup chamado de dentro de um modal tem de ficar acima dele) |
| Removi a ilustração do herói mas deixei o `padding-right`/`min-height` e a grelha de 2 colunas que reservavam o espaço da arte, encolhendo a área dos botões (2+1 com vazio à direita) | Ao remover um elemento, **limpar tudo o que existia só por causa dele** (espaços reservados, colunas de grelha, larguras). Verificar o layout resultante a várias larguras |
| Pus a media query dos botões a decidir "linha vs empilhado" pela largura da **janela** (820px); mas neste shell o espaço útil é a **largura do conteúdo** = janela menos a barra lateral (~286px). Em portátil a janela é >820 mas o conteúdo é estreito, e os botões quebravam 2+1 (só caíam em linha com zoom out) | A responsividade neste shell tem de pensar na **largura do conteúdo**, não da janela. Para N CTAs numa faixa: ou compactar + `nowrap` para caberem na largura de conteúdo de um portátil, ou empilhar; nunca deixar o `flex-wrap` produzir 2+1. **Testar em larguras de portátil reais (com a barra lateral visível)**, não só desktop largo |
| Pediram para corrigir os campos do gerador; corrigi só os `select` (os do 1.º screenshot) e deixei as **checkboxes** deformadas. Entreguei uma correcção **incompleta** | Quando o pedido é "os campos não estão bons", fazer uma **varredura COMPLETA de todos os controlos** do formulário (select, input, **checkbox**, radio, toggle, textarea), não só os do screenshot. Listar cada um e o seu estado antes de dar por terminado |
| As checkboxes financeiras apareciam como "ovo"/pílula vertical. Causa: uma regra `:where(input, select, textarea, ...)` no CSS financeiro punha `min-height:44px` **sem excluir** `[type=checkbox]`/`[type=radio]` → a checkbox nativa (~13px de largura) era esticada para 44px de altura. `min-height` vence `height` mesmo com `!important`, por isso até as que tinham `height:18px` ficavam pílula | Qualquer regra que dimensiona campos (`min-height`/`border-radius` em `input`/`:where(input...)`) **tem de excluir** `[type=checkbox]`, `[type=radio]` e geralmente `[type=hidden]`. Ao auditar, procurar selectores `input` largos sem essa exclusão. Corrigir na **raiz/camada partilhada** (corrigiu as checkboxes de todos os ecrãs financeiros de uma vez) |
| "Editar" recarregava a página (`?edit=ID`) para abrir o modal e recarregava outra vez ao fechar, enquanto "Adicionar" já era pop-up no cliente | Preferir **pop-up no cliente** (sem reload) para abrir/fechar; só a gravação submete. Quando o formulário mapeia **1:1 com as colunas** e a linha vem de `SELECT *`, popular por **nome do campo** (iterar as colunas) é seguro — nenhum campo fica por preencher, logo não há perda de dados ao gravar. Manter o `href ?edit=ID` como **fallback sem-JS** |
| Disse que o scroll do modal estava OK só porque vi `max-height`+`overflow`. Mas vários modais financeiros **não rolavam**: o corpo era `flex:1; overflow:auto` **sem `min-height:0`** → um item flex não encolhe abaixo do seu conteúdo (default `min-height:auto`), transborda o cartão (`max-height`+`overflow:hidden`) e fica cortado, sem nunca activar o scroll | Para um corpo de modal rolável dentro de um cartão flex-coluna com `max-height`: **obrigatório `min-height:0`** no corpo (além de `flex:1;overflow:auto`). Sem isso não rola. Verificar este par em TODOS os sistemas de modal (não bastava `overflow:auto`). Confirmar com conteúdo alto, não só "parece ter as regras certas" |
| Criei a funcionalidade de modelos de crachá num **asset numa pasta NOVA** (`assets/cracha/`). No servidor do utilizador (CloudPanel) a pasta nova **nunca foi criada** ao actualizar → o registo JS nunca carregava (nem por HTTP, nem o fallback `readfile`, pois o ficheiro-fonte também não existia lá). Pré-visualização "indisponível" e impressão em fallback, apesar de o código estar correcto (provado em laboratório) | Pastas novas são um **risco de deploy**: muitos pipelines (CloudPanel, rsync sem novos caminhos, extracção parcial de ZIP) **só substituem ficheiros em pastas existentes**. REGRA acordada com o utilizador: pastas novas **são aceitáveis**, mas é **OBRIGATÓRIO comunicá-las explicitamente** ("esta entrega tem a pasta NOVA `X/` — crie-a/deploye-a à mão"), para ele fazer o deploy manual com cuidado. Em alternativa, usar **pastas já existentes** (`assets/views/`) ou **embutir**. Em qualquer caso, **confirmar a entrega real** (ver-fonte/Console: `typeof window.X` distingue "não-deployado" de "deployado-mas-com-bug"), não assumir que o ZIP basta |
