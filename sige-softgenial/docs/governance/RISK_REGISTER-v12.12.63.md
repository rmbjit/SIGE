# RISK_REGISTER - v12.12.63 - MFA de Operacao (correccao pos-rediagnostico)
...
Ajuste fino final da reconciliacao viva: classificacao por operadora, com defaults fundamentados e sobreposicao por filtro.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Apenas afina a classificacao da verificacao (so leitura); nao toca em regras de calculo nem em ficheiros canonicos. |
| P1 | Nenhum. Por operadora, a verificacao fica mais exacta; o isolamento impede que um codigo ou estado de falha de uma operadora contamine a outra. |
| P2 | Nenhum novo. Risco de acusar uma transacao real de falsa continua eliminado: estado nao mapeado -> desconhecida, e a falha so e afirmada por estado ou codigo conhecido da propria operadora. |
| P3 | Nenhum novo. A mudanca e retro-compativel (sem provider, defaults comuns; filtros antigos de um argumento continuam a funcionar). |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem dependencias novas (11), sem alteracao de esquema, calculo byte-identico. Corredor 95/95. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.45 - Fase 4 incr 1 - Self-host das bibliotecas e infra de enqueue

Self-host das bibliotecas criticas com SRI, catalogo repontado para local, novo registador de enqueue, e exclusao de assets/vendor/ do scan de governanca (deps 11 -> 9).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Camada habilitadora; nao toca em regras de calculo nem em ficheiros canonicos. As bibliotecas sao as mesmas versoes, agora locais e com SRI verificavel. |
| P1 | Nenhum. Self-host reduz a superficie de ataque (sem dependencia de CDN de terceiros) e mantem a integridade via SRI recalculado a partir dos ficheiros entregues. |
| P2 | Nenhum novo. Os treze ecrans que usam sige_cdn_script continuam a funcionar sem alteracao; a portaria e o fallback do bootstrap deixam de referir unpkg. |
| P3 | Nenhum novo. O registador apenas regista handles (nao enfileira por si so); a migracao do inline fica para as vagas seguintes, sem mudanca de comportamento agora. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico. Dependencias externas 11 -> 9. Corredor 97. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.46 - Fase 4 incr 2 - Migracao de inline (vaga 1) e catraca

Despachante data-sige-act (delegacao reutilizavel), primeira vaga de migracao de onclick e gate de catraca do inline.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. So muda a forma de ligar o clique a funcao; nao toca em regras de calculo nem em ficheiros canonicos. |
| P1 | Nenhum. A logica dos handlers e identica; o despachante apenas chama a funcao global, com o id por data-sige-arg ou o elemento por omissao. |
| P2 | Nenhum novo. A catraca impede que o inline aumente; cada vaga futura so o baixa. |
| P3 | Nenhum novo. O despachante em bolha nao colide com o enhancer de confirmacao em captura; sem JS por ecra. |

Aditivo. Sem alteracao de superficie (199/33), vista (60), opcoes (132) nem dependencias (9), sem alteracao de esquema, calculo byte-identico. Corredor 98. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.47 - Fase 4 incr 2 vaga 2 - Estilos academicos para utilitarios

Migracao do estilo inline da area academica para classes utilitarias com !important, sob a catraca que garante que o inline so desce (style 2191 -> 2142).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de interface; nao toca em regras de calculo nem em ficheiros canonicos. So muda a forma como o estilo e aplicado (classe em vez de atributo inline), com a mesma especificidade. |
| P1 | Nenhum. Os utilitarios usam !important para replicar a especificidade do inline (1000), pelo que nao ha regressao de cascata; o aspecto visual e identico. |
| P2 | Nenhum novo. A migracao e conservadora: so atributos 100 por cento seguros, sem PHP e sem class existente; os restantes casos ficam para vagas futuras. Sem class duplicada nas vistas migradas. |
| P3 | Nenhum novo. A catraca consolidada (um unico gate) e o smoke no corredor impedem que o inline volte a subir. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de style 2191 -> 2142. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.48 - Fase 4 incr 2 vaga 3 - Estilos financeiros para utilitarios

Migracao do estilo inline da area financeira para classes utilitarias com !important, sob a catraca que garante que o inline so desce (style 2142 -> 2110).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de interface; nao toca em regras de calculo financeiro nem em ficheiros canonicos. So muda a forma como o estilo e aplicado (classe em vez de atributo inline), com a mesma especificidade. |
| P1 | Nenhum. Os utilitarios usam !important para replicar a especificidade do inline (1000); nao ha regressao de cascata e o aspecto e identico. A guarda de scroll movida para sige-u-oxa e reconhecida pelos dois gates responsivos. |
| P2 | Nenhum novo. So atributos 100 por cento seguros, sem PHP e sem class existente; os restantes ficam para vagas futuras. Sem class duplicada nas vistas migradas. |
| P3 | Nenhum novo. A catraca (style 2110, sem folga) e os gates responsivos atualizados impedem regressao. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de style 2142 -> 2110. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.49 - Fase 4 incr 2 - Fixe de impressao utilitarios e vaga admin-shell e jardim

Correccao de impressao das classes utilitarias (helper embebido em 5 popups, mais guarda no smoke) e migracao de 59 estilos inline no admin-shell e jardim, sob a catraca (style 2110 -> 2051).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Correccao de apresentacao no impresso e vaga de interface; nao toca em regras de calculo nem em ficheiros canonicos. |
| P1 | Nenhum. Antes desta versao, as classes utilitarias migradas nas vagas 2 e 3 ficavam sem efeito no impresso de 5 vistas com popup autonomo (regressao cosmetica). Fica resolvido com o helper embebido em cada popup; a guarda do smoke impede recaida. O admin-shell migrou icones (flex-shrink) sem regressao de cascata (utilitarios com !important). |
| P2 | Nenhum novo. Migracao so de atributos 100 por cento seguros, sem PHP e sem class existente; ficheiros de saida autonoma e canonicos excluidos por desenho; sem class duplicada. |
| P3 | Nenhum novo. A catraca (style 2051, sem folga) e a guarda anti-regressao de impressao impedem novos casos. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de style 2110 -> 2051. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.50 - Fase 4 incr 2 - Onclick para data-sige-act (vistas limpas)

Inicio da migracao de onclick para data-sige-act, com despachante fiel (data-sige-arg / data-sige-noargs / elemento) e 49 conversoes seguras, sob a catraca (onclick 250 -> 201).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de interface; nao toca em regras de calculo nem em ficheiros canonicos. So muda a forma como o handler e ligado (atributo declarativo em vez de onclick), sem alterar a logica. |
| P1 | Nenhum. So se converteram onclick sem risco: fn() passa a data-sige-noargs (chama fn() sem o elemento, fiel ao original, mesmo quando a funcao usa o 1o parametro, como plToggleNovoPlano); fn(this) passa o elemento; fn('texto') passa o texto. O despachante e retro-compativel com a vaga 1. |
| P2 | Nenhum novo. Vistas com popup, templates de PDF, portal e canonicos ficam de fora; onclick com numeros, multiplos argumentos ou PHP interpolado ficam para vagas futuras com despachante mais rico. |
| P3 | Nenhum novo. A catraca (onclick 201, sem folga) e a verificacao do contrato do despachante impedem regressao. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 250 -> 201. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.51 - Fase 4 incr 2 - Onclick complexos para data-sige-args (despachante mais rico)

Migracao dos onclick complexos (numeros, booleanos, multi-arg, PHP) para o despachante rico (data-sige-args JSON mais data-sige-self), com 20 conversoes seguras, sob a catraca (onclick 201 -> 181).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de interface; nao toca em regras de calculo nem em ficheiros canonicos. So muda a forma como o handler e ligado (atributo declarativo em vez de onclick), sem alterar a logica. |
| P1 | Nenhum. O ponto delicado (PHP no atributo) e tratado com esc_attr(wp_json_encode([...])), que escapa aspas, e comercial, maior e menor e acentos; a simulacao de renderizacao confirma JSON.parse fiel ao valor original, inclusive com nomes contendo aspas e simbolos. O despachante e retro-compativel com as vagas anteriores; um JSON invalido nao dispara a funcao. |
| P2 | Nenhum novo. Expressoes inline (window.print, this.style, this.classList, IIFE, jQuery, window.open(this.href), window.location) e onclick gerados em template JS ficam para a vaga de funcoes nomeadas; uma chamada de quatro argumentos fica diferida. |
| P3 | Nenhum novo. A catraca (onclick 181, sem folga) e a verificacao do contrato rico do despachante impedem regressao. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 201 -> 181. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.52 - Fase 4 incr 2 - Expressoes inline em onclick para funcoes nomeadas

Conversao de 10 expressoes inline em onclick para sete funcoes nomeadas globais, ligadas pelo despachante data-sige-act, sob a catraca (onclick 181 -> 171).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de interface; nao toca em regras de calculo nem em ficheiros canonicos. So muda a forma como a logica de interface e ligada (funcao nomeada em vez de expressao na etiqueta). |
| P1 | Nenhum. Cada funcao nomeada e fiel a expressao que substitui; recebe o elemento (equivalente ao this) quando a expressao o usava, ou nada (data-sige-noargs) quando nao usava. O menu lateral movel do shell e os botoes de imprimir e de alternar continuam a comportar-se como antes. |
| P2 | Nenhum novo. Expressoes com navegacao (window.open(this.href), window.location), cadeias jQuery e IIFE, onclick gerados em template JS, e paginas autonomas ficam para vagas proprias. |
| P3 | Nenhum novo. A catraca (onclick 171, sem folga) impede regressao. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 181 -> 171. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.53 - Fase 4 incr 2 - Onclick de navegacao e template JS para funcoes nomeadas

Conversao de 7 onclick de navegacao e template JS para funcoes nomeadas, com data-sige-prevent, sob a catraca (onclick 171 -> 164).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de interface; nao toca em regras de calculo nem em ficheiros canonicos. So muda a forma como a logica de interface e ligada. |
| P1 | Nenhum. O return false dos links de recibo e substituido por data-sige-prevent (preventDefault), mantendo o comportamento de abrir o recibo numa janela sem navegar; o href fica preservado para acessibilidade. O mailto e os botoes de adiar continuam a funcionar; os onclick de template JS preservam a interpolacao do id e o elemento. As regras anteriores do despachante ficam inalteradas. |
| P2 | Nenhum novo. As cadeias jQuery e IIFE, os ficheiros com janela autonoma ou popups, e os canonicos ficam para vagas proprias. |
| P3 | Nenhum novo. A catraca (onclick 164, sem folga) e a verificacao do contrato data-sige-prevent impedem regressao. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 171 -> 164. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.54 - Fase 4 incr 2 - Cadeias jQuery e IIFE em onclick reescritas em vanilla

Reescrita de 2 cadeias jQuery/IIFE em onclick para vanilla, em funcoes nomeadas co-localizadas, sob a catraca (onclick 164 -> 162). Conclui os onclick em ficheiros limpos.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de interface; nao toca em regras de calculo nem em ficheiros canonicos. So muda a forma como a logica de interface e ligada. |
| P1 | Nenhum. As funcoes vanilla sao fieis as cadeias originais: o painel de pendentes reabre como antes; o modal de clonagem fecha, marca aria-hidden e retira a classe do corpo se nenhum modal estiver visivel, com o teste de visibilidade do jQuery reproduzido por offsetWidth, offsetHeight e getClientRects. As funcoes estao co-localizadas nas vistas e expostas em window. |
| P2 | Nenhum novo. Os onclick em ficheiros com janela autonoma, em popups ou canonicos ficam para abordagem propria. |
| P3 | Nenhum novo. A catraca (onclick 162, sem folga) impede regressao; nao resta jQuery ou IIFE em onclick de ficheiros limpos. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 164 -> 162. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.55 - Fase 4 incr 2 - Infraestrutura de nonce CSP e nonce nas tags script inline

Introducao do nonce CSP por pedido (core-helpers) e aplicacao a 55 tags script inline das vistas, sob a catraca (blocos script 81 -> 25), a preparar o enforcement do CSP.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de preparacao para o CSP; nao toca em regras de calculo nem no ficheiro canonico finance-core. Acrescentar o nonce e inocuo sem cabecalho CSP. |
| P1 | Nenhum. O nonce e gerado por pedido e disponivel em todos os pedidos onde o plugin carrega (core-helpers e carregado incondicionalmente no bootstrap, antes de qualquer vista renderizar). As tags convertidas continuam a comportar-se como antes; o auxiliar tem recurso a wp_generate_password se random_bytes falhar. |
| P2 | Nenhum novo. Os redireccionamentos echo, as paginas autonomas, o portal publico, o gerador CDN e o finance-core ficam para abordagem propria; a pagina de login fica de fora por ser pre-autenticacao. |
| P3 | Nenhum novo. A catraca (blocos script 25, sem folga) impede regressao e passa a exigir a infraestrutura de nonce. Os verificadores de JS embebido normalizam a tag com nonce antes de extrair. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de blocos script 81 -> 25. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.56 - Fase 4 incr 2 - Nonce CSP nos redireccionamentos echo

Aplicacao do nonce CSP a 11 redireccionamentos construidos por echo nas vistas autenticadas, sob a catraca (blocos script 25 -> 14), a continuar a preparacao do enforcement do CSP.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de preparacao para o CSP; nao toca em regras de calculo nem no ficheiro canonico finance-core. Acrescentar o nonce e inocuo sem cabecalho CSP. |
| P1 | Nenhum. Os redireccionamentos estao em vistas autenticadas, renderizadas apos o bootstrap, onde o auxiliar de nonce esta disponivel. A conversao so altera a tag de abertura; o comportamento e identico (o nonce e ignorado sem cabecalho CSP). |
| P2 | Nenhum novo. As paginas autonomas, o portal publico, o finance-core, a pagina de login (pre-auth) e o popup document.write ficam para abordagem propria. |
| P3 | Nenhum novo. A catraca (blocos script 14, sem folga) impede regressao. Os verificadores de JS embebido mantem-se verdes porque o conteudo extraido nao muda. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de blocos script 25 -> 14. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.57 - Fase 4 incr 2 - Nonce CSP nas paginas autonomas

Aplicacao do nonce CSP as 7 tags script das paginas autonomas, sob a catraca (blocos script 14 -> 7), concluindo a aplicacao do nonce a todos os scripts inline alcancaveis.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de preparacao para o CSP; nao toca em regras de calculo nem no ficheiro canonico finance-core. Acrescentar o nonce e inocuo: nao ha CSP a referenciar o nonce nestas paginas e o CSP existente do motor de documentos mantem unsafe-inline. |
| P1 | Nenhum. As paginas renderizam apos o bootstrap (templates incluidos por handlers, vista do jardim e includes do motor de documentos e da camara), onde o auxiliar de nonce esta disponivel. Os cabecalhos de seguranca existentes ficaram intactos; os botoes de impressao (onclick) continuam a funcionar porque o CSP do motor de documentos mantem unsafe-inline. |
| P2 | Nenhum novo. O enforcement do CSP nestas paginas depende de converter os onclick inline (botoes de impressao) e fica para incremento dedicado. O portal publico, o finance-core, a pagina de login (pre-auth) e o popup document.write ficam para abordagem propria. |
| P3 | Nenhum novo. A catraca (blocos script 7, sem folga) impede regressao. Os verificadores de JS embebido mantem-se verdes (normalizam a tag com nonce). |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de blocos script 14 -> 7. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.58 - Fase 4 incr 2 - onclick de impressao das autonomas em addEventListener

Conversao de 13 onclick de impressao das paginas autonomas para addEventListener local, sob a catraca (onclick 162 -> 149), preparando o CSP enforce nessas paginas.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Vaga de preparacao; nao toca em regras de calculo nem no ficheiro canonico finance-core. Comportamento dos botoes identico. |
| P1 | Nenhum. A ligacao corre ao nivel de topo do script (fora dos IIFE de QRious), com o script depois dos botoes no DOM, pelo que os botoes ficam sempre ligados; o imprimir um boletim preserva o tipo numero com parseInt. Nenhum cabecalho de seguranca existente foi tocado. |
| P2 | Nenhum novo. O CSP enforce e a substituicao de unsafe-inline por nonce ficam para o incremento seguinte. O portal publico, o finance-core, a pagina de login (pre-auth) e o popup document.write ficam para abordagem propria. |
| P3 | Nenhum novo. A catraca (onclick 149, sem folga) impede regressao. Os verificadores de JS embebido mantem-se verdes. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 162 -> 149. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.59 - Fase 4 incr 2 - CSP enforce com nonce nas paginas autonomas

Primeiro CSP enforce com nonce, nas paginas autonomas, com asserccao na catraca a proibir a regressao a unsafe-inline no script-src.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Nao toca em regras de calculo nem no ficheiro canonico finance-core. O script-src self mais nonce nao bloqueia nada porque as paginas tem zero handlers inline, scripts inline com nonce e scripts externos da propria origem. |
| P1 | Nenhum. O nonce do cabecalho casa exactamente com o das tags; cada CSP esta isolado a sua pagina (a funcao da camara guarda o pedido, os handlers so correm na impressao); a validacao da camara vai para admin-ajax na propria origem (connect-src self). Esta e a primeira vaga que altera comportamento nesta frente, e foi verificada recurso a recurso. |
| P2 | Nenhum novo. O CSP enforce das vistas admin (que ainda tem onclick e estilos inline) e um esforco grande a parte. O portal publico, o finance-core, a pagina de login e o popup document.write ficam para abordagem propria. |
| P3 | Nenhum novo. A catraca exige script-src com nonce nas paginas autonomas e proibe unsafe-inline no script-src, travando a regressao. |

Aditivo em superficie. Altera comportamento (script-src enforce). Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.60 - Fase 4 incr 2 - onclick seguros das vistas admin em data-sige-act

Conversao de 56 onclick seguros das vistas admin para o despachante data-sige-act, sob a catraca (onclick 149 -> 93), preparando o CSP enforce do shell admin.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Nao toca em regras de calculo nem no ficheiro canonico finance-core. Usa o despachante data-sige-act ja em producao; comportamento dos botoes identico. |
| P1 | Nenhum. So foi convertido o subconjunto inequivocamente seguro (sem-arg e inteiro); os de inteiro preservam o tipo numero via data-sige-args; o conversor mascarou os blocos script, poupando os onclick gerados por JavaScript. Os difíceis (event, codigo inline, jQuery, string esc_js) ficaram intactos. |
| P2 | Nenhum novo. O CSP enforce do shell admin so vem depois de tratados os onclick e estilos inline restantes. O portal publico, o finance-core e a pagina de login ficam para abordagem propria. |
| P3 | Nenhum novo. A catraca (onclick 93, sem folga) impede regressao. Os verificadores de JS embebido mantem-se verdes. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 149 -> 93. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.61 - Fase 4 incr 2 - uploadDoc e fecho de modais das vistas admin em data-sige-act

Conversao de 14 onclick seguros das vistas admin para o despachante data-sige-act, sob a catraca (onclick 93 -> 79), continuando a preparar o CSP enforce do shell admin.

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Nao toca em regras de calculo nem no ficheiro canonico finance-core. Usa o despachante data-sige-act ja em producao; comportamento identico. |
| P1 | Nenhum. So foram convertidos padroes inequivocamente seguros (uploadDoc com string estatico e fecho de modais jQuery via wrapper fiel); o conversor mascarou os blocos script. O switchTab ficou intacto. |
| P2 | Nenhum novo. O CSP enforce do shell admin so vem depois de tratados os onclick e estilos inline restantes. |
| P3 | Nenhum novo. A catraca (onclick 79, sem folga) impede regressao. Verificadores de JS embebido verdes; sige-ui.js passa node -c. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 93 -> 79. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.62 - Fase 4 incr 2 - switchTab e args com esc_js das vistas admin em data-sige-act

Conversao de 13 onclick das vistas admin que exigiam cuidado proprio (switchTab e argumentos string via esc_js) para o despachante data-sige-act, sob a catraca (onclick 79 -> 66).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Nao toca em regras de calculo nem no ficheiro canonico finance-core. Usa o despachante data-sige-act ja em producao; comportamento identico. |
| P1 | Nenhum. O switchTab le o tabId do data-tab que as abas ja possuem, e como sao div sem filhos o event.target era ja o elemento; os argumentos string sao serializados por esc_attr(wp_json_encode), com round-trip testado ate JSON.parse para um valor dificil, preservando tipos. As funcoes alvo sao globais. |
| P2 | Nenhum novo. O CSP enforce do shell admin so vem depois de tratados os onclick e estilos inline restantes. |
| P3 | Nenhum novo. A catraca (onclick 66, sem folga) impede regressao. Verificadores de JS embebido verdes. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 79 -> 66. Corredor 99. Rediagnostico adversarial Zero P0/P1.


## Actualizacao v12.12.63 - Fase 4 incr 2 - CSP Report-Only no shell admin mais grande lote de onclick admin em data-sige-act

Activacao do CSP Report-Only no shell admin (script-src nonce, nao bloqueia) e conversao de 21 onclick das vistas admin para o despachante data-sige-act, sob a catraca (onclick 66 -> 45).

| Severidade | Estado nesta versao |
|---|---|
| P0 | Nenhum. Nao toca em regras de calculo nem no ficheiro canonico finance-core. O cabecalho e Report-Only, nao bloqueia nada; as conversoes usam o despachante ja em producao; comportamento identico. |
| P1 | Nenhum. Report-Only e o caminho seguro para o enforce numa superficie grande nao testavel em browser daqui: nada e bloqueado e recolhem-se as violacoes reais. O cabecalho so vai em page=sige-app, antes do output, fora de ajax. Os 2 onclick canonicos de finance-core estao numa pagina standalone, fora do shell. As conversoes preservam tipos e elemento. |
| P2 | Nenhum novo. Faltam, para o enforce, os onclick window.open(this.href), this.form e os gerados por JS, que o proprio Report-Only reporta. Os estilos inline ficam para o eixo do style-src. |
| P3 | Nenhum novo. A catraca (onclick 45, sem folga) e a asserccao do Report-Only impedem regressao. Verificadores de JS embebido verdes; sige-ui.js passa node -c. |

Aditivo. Sem nova superficie (199/33), sem nova vista (60), sem opcoes novas (132), sem alteracao de esquema, calculo byte-identico, deps 9. Catraca de onclick 66 -> 45. Corredor 99. Rediagnostico adversarial Zero P0/P1.
