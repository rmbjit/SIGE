# TRACEABILITY_MATRIX - v12.12.63 - MFA de Operacao (Step-up)
...
| ID | Criterio | Evidencia |
|---|---|---|
| CSPE-001 | Conversao correcta | 11 redireccionamentos convertidos por concatenacao em 6 vistas (dashboard, disciplinas, encerramento, financeiro-config, notas, transporte), tratando aspa simples e dupla |
| CSPE-002 | Tag valida com nonce | render simulado produz script com atributo nonce base64 e sem script puro, em ambos os tipos de aspa |
| CSPE-003 | Nonce por pedido | o mesmo nonce do core-helpers e usado nos redireccionamentos e nos restantes scripts do pedido |
| CSPE-004 | Inocuidade | os redireccionamentos continuam a redireccionar como antes (o nonce e ignorado sem cabecalho CSP); php -l limpo nas 6 vistas |
| CSPE-005 | Verificadores de JS intactos | a conversao so altera a tag de abertura, nao o conteudo extraido; check-js-views e check-js-views-combined mantem-se verdes sem alteracao |
| CSPE-006 | Exclusoes | paginas autonomas (PDF, documentos, camara), portal, finance-core, login (pre-auth) e popup document.write ficam de fora, por desenho |
| CSPE-007 | Catraca travada | check-inline-frontend com blocos script max 14 (sem folga) |
| CSPE-008 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |

Evidencia consolidada: tools/check-inline-frontend.php, tools/check-js-views.php, tools/check-js-views-combined.php, tools/smoke-inline-frontend.php, tools/smoke-regression-pack.php verdes; teste de render dos redireccionamentos; gates de pagamentos, cofre, Fase 1 e Fase 4 incr 1 verdes; corredor 99; smoke-release-gate verde a partir de pasta limpa.
...
| ID | Criterio | Evidencia |
|---|---|---|
| CSPA-001 | Conversao das paginas autonomas | 7 tags convertidas em 5 ficheiros (pauta-pdf-template, boletim-pdf-template, jardim_boletim-view, documents-engine, portaria-camera-safe-page) |
| CSPA-002 | Tags com nonce | as 7 tags passam a forma <script <?php echo sige_csp_script_attr(); ?>>, renderizando script com nonce base64 |
| CSPA-003 | Disponibilidade | as paginas renderizam apos o bootstrap; os templates PDF sao incluidos por handlers (pauta-pdf-handler, boletim-pdf-handler), a camara intercepta um pedido frontend ja com o plugin carregado |
| CSPA-004 | Inocuidade | nao ha CSP a referenciar o nonce nestas paginas; no motor de documentos o CSP mantem unsafe-inline sem nonce na directiva, pelo que os scripts e os onclick de impressao continuam a funcionar; php -l limpo nos 5 ficheiros |
| CSPA-005 | Cabecalhos existentes intactos | os tres Content-Security-Policy do motor de documentos e o Permissions-Policy da camara mantem-se inalterados |
| CSPA-006 | Verificadores de JS intactos | os modelos PDF em admin sao varridos e continuam verdes (a tag com nonce e normalizada para extraccao desde a v12.12.55) |
| CSPA-007 | Dependencia conhecida para o enforce | estas paginas usam onclick inline (botoes de impressao); o enforcement do CSP exige converter esses onclick e fica para incremento dedicado |
| CSPA-008 | Catraca travada | check-inline-frontend com blocos script max 7 (sem folga); conclui a aplicacao do nonce a todos os scripts inline alcancaveis |
| CSPA-009 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |

Evidencia consolidada: tools/check-inline-frontend.php, tools/check-js-views.php, tools/check-js-views-combined.php, tools/smoke-inline-frontend.php, tools/smoke-regression-pack.php verdes; gates de pagamentos, cofre, Fase 1 e Fase 4 incr 1 verdes; corredor 99; smoke-release-gate verde a partir de pasta limpa.
...
| ID | Criterio | Evidencia |
|---|---|---|
| OCKA-001 | Conversao dos onclick | 13 handlers convertidos em 4 paginas (pauta-pdf-template 2, boletim-pdf-template 2, jardim_boletim-view 2, documents-engine 7); a camara ja nao tinha handlers inline |
| OCKA-002 | Ligacao local | os botoes passam a atributos data e a ligacao e feita por addEventListener dentro do script ja com nonce de cada pagina, ao nivel de topo (fora dos IIFE de QRious) |
| OCKA-003 | Tipo preservado | o imprimir um boletim passa o indice por atributo data e reconverte-o a numero com parseInt, preservando o tipo esperado pela funcao sigeImprimirBoletins |
| OCKA-004 | Comportamento identico | os botoes continuam a imprimir, fechar e imprimir boletins como antes; php -l limpo nos 4 ficheiros |
| OCKA-005 | Robustez | a ligacao corre mesmo sem a biblioteca de QR, porque esta fora dos IIFE que retornam cedo por falta de QRious |
| OCKA-006 | Cabecalhos existentes intactos | os tres Content-Security-Policy do motor de documentos e o Permissions-Policy da camara mantem-se inalterados |
| OCKA-007 | Verificadores de JS intactos | os modelos PDF em admin sao varridos e continuam verdes |
| OCKA-008 | Catraca travada | check-inline-frontend com onclick max 149 (sem folga) |
| OCKA-009 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |
...
| ID | Criterio | Evidencia |
|---|---|---|
| CSPF-001 | script-src nonce no motor de documentos | os tres cabecalhos Content-Security-Policy passam script-src de unsafe-inline para self mais nonce por pedido |
| CSPF-002 | Cabecalho CSP nos modelos PDF | pauta-pdf-handler e boletim-pdf-handler enviam o CSP com nonce antes do output, a seguir ao Content-Type |
| CSPF-003 | Cabecalho CSP no boletim do jardim | enviado no topo da vista, protegido por headers_sent, antes do DOCTYPE |
| CSPF-004 | Cabecalho CSP na camara | acrescentado a funcao de cabecalhos da camara, que so corre no pedido da camara; connect-src self, media-src self, img-src self data https |
| CSPF-005 | Nonce do cabecalho casa com o das tags | ambos de sige_csp_nonce; esc_attr nao altera o base64; teste confirma igualdade exacta |
| CSPF-006 | Nada bloqueado | zero handlers inline e zero URLs javascript nas paginas; scripts inline com nonce; scripts externos (QRious, html5-qrcode) servidos da propria origem |
| CSPF-007 | Validacao da camara coberta | fetch para admin-ajax na propria origem, coberta por connect-src self |
| CSPF-008 | Isolamento | cada CSP so e emitido na sua pagina, sem fuga para outras |
| CSPF-009 | Catraca trava regressao | check-inline-frontend exige script-src com nonce nas paginas autonomas e proibe unsafe-inline no script-src |
| CSPF-010 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |

Evidencia consolidada: tools/check-inline-frontend.php, tools/check-js-views.php, tools/check-js-views-combined.php, tools/smoke-inline-frontend.php, tools/smoke-regression-pack.php verdes; teste de igualdade do nonce; gates de pagamentos, cofre, Fase 1 e Fase 4 incr 1 verdes; corredor 99; smoke-release-gate verde a partir de pasta limpa.
...
| ID | Criterio | Evidencia |
|---|---|---|
| OADM-001 | Conversao do subconjunto seguro | 52 onclick sem-arg (data-sige-noargs) e 4 onclick inteiro (data-sige-args com lista JSON) em 8 vistas admin |
| OADM-002 | Mecanismo correcto | as vistas admin correm no shell que carrega o despachante data-sige-act; usou-se esse mecanismo, ja em producao e validado pelos gates |
| OADM-003 | Tipo preservado | os onclick de inteiro passam a data-sige-args com a lista JSON, que o despachante interpreta preservando o tipo numero |
| OADM-004 | Difíceis poupados | o conversor mascara os blocos script; os onclick gerados por JavaScript nao foram tocados; os de event, codigo inline, jQuery e string esc_js ficaram intactos |
| OADM-005 | Comportamento identico | o despachante chama a mesma funcao com os mesmos argumentos; php -l limpo nas 8 vistas |
| OADM-006 | Verificadores de JS intactos | as vistas admin sao varridas e continuam verdes |
| OADM-007 | Despachante integro | smoke-inline-frontend confirma o contrato data-sige-noargs/data-sige-args/data-sige-arg/data-sige-self/data-sige-prevent |
| OADM-008 | Catraca travada | check-inline-frontend com onclick max 93 (sem folga) |
| OADM-009 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |

Evidencia consolidada: tools/check-inline-frontend.php, tools/smoke-inline-frontend.php, tools/check-js-views.php, tools/check-js-views-combined.php, tools/smoke-regression-pack.php verdes; gates de pagamentos, cofre, Fase 1 e Fase 4 incr 1 e 2 verdes; corredor 99; smoke-release-gate verde a partir de pasta limpa.
...
| ID | Criterio | Evidencia |
|---|---|---|
| OAD2-001 | Conversao uploadDoc | 6 onclick uploadDoc com string estatico passam a data-sige-act mais data-sige-arg |
| OAD2-002 | Conversao fecho de modais | 8 onclick jQuery fadeOut passam a data-sige-act mais data-sige-arg com o wrapper sigeFecharModalJq |
| OAD2-003 | Wrapper fiel e seguro | sigeFecharModalJq faz jQuery(sel).fadeOut(200) com guarda de jQuery, seguindo o padrao das funcoes globais ja expostas para o despachante |
| OAD2-004 | window.print ausente | zero onclick window.print() nas vistas admin fora de blocos script |
| OAD2-005 | switchTab poupado | mantido intacto para incremento proprio por usar event.target |
| OAD2-006 | Difíceis poupados | o conversor mascara os blocos script; os onclick gerados por JavaScript nao foram tocados |
| OAD2-007 | Comportamento identico | uploadDoc recebe a mesma cadeia; o fecho de modal faz o mesmo fadeOut; php -l limpo nas 3 vistas; sige-ui.js passa node -c |
| OAD2-008 | Despachante integro | smoke-inline-frontend confirma o contrato; o wrapper novo nao quebra o gate |
| OAD2-009 | Catraca travada | check-inline-frontend com onclick max 79 (sem folga) |
| OAD2-010 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |
...
| ID | Criterio | Evidencia |
|---|---|---|
| OAD3-001 | switchTab refeito | switchTab(event, tabId) passa a switchTab(el) lendo o tabId do data-tab; os 4 call-sites passam a data-sige-act (via por omissao passa o elemento) |
| OAD3-002 | switchTab seguro | as abas sao div sem filhos, pelo que event.target era ja o elemento; switchTab so e chamado por esses 4 onclick, nao programaticamente |
| OAD3-003 | Args esc_js serializados | 9 onclick com argumentos string via esc_js passam a data-sige-args com esc_attr(wp_json_encode([...])), preservando inteiros, decimais e cadeias |
| OAD3-004 | Round-trip testado | testou-se esc_attr(wp_json_encode) ate JSON.parse com aspa simples, aspa dupla, menor e e-comercial; recuperou-se o valor exacto |
| OAD3-005 | Funcoes globais | cada funcao alvo e global (funcao ou window.nome), encontrada pelo despachante via window |
| OAD3-006 | Comportamento identico | as abas trocam como antes; os botoes recebem os mesmos argumentos; php -l limpo nas 5 vistas |
| OAD3-007 | Verificadores de JS intactos | as vistas admin sao varridas e continuam verdes; despachante integro (smoke-inline-frontend) |
| OAD3-008 | Catraca travada | check-inline-frontend com onclick max 66 (sem folga) |
| OAD3-009 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |
...
| ID | Criterio | Evidencia |
|---|---|---|
| ROAD-001 | Report-Only no shell admin | admin_init em page=sige-app envia Content-Security-Policy-Report-Only com script-src self mais nonce, antes do output, protegido por headers_sent e nao-ajax |
| ROAD-002 | Nao bloqueia | sendo Report-Only, o admin funciona na integra e a consola reporta os handlers inline e scripts sem nonce que faltam tratar |
| ROAD-003 | Foco no script | so script-src e restringido; os restantes tipos ficam sem restricao, para a consola so reportar o que interessa |
| ROAD-004 | Canonico fora do shell | os 2 onclick de finance-core estao em sige_desp_print_page, pagina standalone de impressao, fora do shell admin |
| ROAD-005 | Lote convertido | 21 onclick das vistas admin em data-sige-act: 18 com contratos/wrappers existentes, 3 com 2 wrappers novos |
| ROAD-006 | Wrappers novos | sigeAlternarDisplay (data-target, data-display) e sigeMarcarDownloadSemTransicao; sige-ui.js passa node -c |
| ROAD-007 | Comportamento identico | os botoes chamam as mesmas funcoes; php -l limpo nos ficheiros tocados |
| ROAD-008 | Catraca e asserccao | check-inline-frontend com onclick max 45 (sem folga) e exigencia do Report-Only com script-src nonce no shell |
| ROAD-009 | Despachante integro | smoke-inline-frontend confirma o contrato; os wrappers novos nao o quebram |
| ROAD-010 | Sem regressao de invariantes | smoke-regression-pack verde; superficie 199, opcoes 132, vistas 60, deps 9; calculo byte-identico |
