...
// [v12.32.0] Configuração do modelo de crachá por escola (definicoes + AJAX).
// Depende de sige_get_escola_id; sem schema, so WP options por escola.
if (file_exists(SIGE_PATH . 'includes/cracha-config.php')) {
    require_once SIGE_PATH . 'includes/cracha-config.php';
}

// [v12.34.0] Recursos Humanos - alertas (contratos a expirar). So leitura de
...
// [v12.38.0] Recursos Humanos - Ferias & Ausencias (Fase 1). Tabela propria
// (sige_rh_ausencias) criada por codigo de forma idempotente; gestao pelo RH.
// Nao cria permissoes novas nem toca em ficheiros protegidos.
if (file_exists(SIGE_PATH . 'includes/rh-ausencias.php')) {
    require_once SIGE_PATH . 'includes/rh-ausencias.php';
}

// [v12.39.0] Recursos Humanos - Assiduidade / Ponto (Fase 1). Tabela propria
// (sige_rh_assiduidade) criada por codigo; marcacao diaria integrada com as
// ausencias aprovadas. Alimenta o Processamento de Salario (fase seguinte).
if (file_exists(SIGE_PATH . 'includes/rh-assiduidade.php')) {
    require_once SIGE_PATH . 'includes/rh-assiduidade.php';
}

// [v12.40.0] Recursos Humanos - Processamento de Salario (Fase 1). Tabela propria
// (sige_rh_salarios) + config de impostos EDITAVEL por escola (INSS/IRPS). So RH.
// Consome faltas da Assiduidade. Nao toca no modulo financeiro nem em protegidos.
if (file_exists(SIGE_PATH . 'includes/rh-salarios.php')) {
    require_once SIGE_PATH . 'includes/rh-salarios.php';
}
